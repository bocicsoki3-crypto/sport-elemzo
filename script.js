...
const data = await response.json();
        ...
        appState.currentAnalysisContext = data.html;
// Feltételezzük, hogy a válasz { html: "..." }
...
@@ -264,64 +209,82 @@ const response = await fetch(appState.gasUrl, {
        if (data.error) throw new Error(data.error);
...
{
        switch (data.signal) {
            case 'GREEN': signalColor = 'var(--success)';
signalText = 'ZÖLD JELZÉS ✅'; break;
...
function handleSportChange() {
    ...
    appState.selectedMatches.clear();
// Töröljük a kiválasztást sportváltáskor
    appState.selectedMatches.clear();
...
@@ -269,26 +308,24 @@ function getLeagueGroup(leagueName) {
...
const groupedByCategory = groupBy(fixtures, fx => getLeagueGroup(fx.league));
    ...
    groupOrder.forEach(group => {
        let columnContent = '';
@@ -331,516 +294,481 @@ const groupedByCategory = groupBy(fixtures, fx => getLeagueGroup(fx.league));
            ...
            Object.keys(groupedByDate).sort((a, b) => new Date(a.split('. ').join('.').split('.').reverse().join('-')) - new Date(b.split('. ').join('.').split('.').reverse().join('-'))).forEach(dateKey => {
                ...
                // === JAVÍTÁS (6. KÉP): Meccsek sorbarendezése idő szerint a napon belül ===
...
@@ -319,22 +350,19 @@ function renderFixturesForDesktop(fixtures) {
...
groupOrder.forEach(group => {
    groupOrder.forEach(group => {
        if (groupedByCategory[group]) {
            ...
            // === JAVÍTÁS (6. KÉP): Meccsek sorbarendezése idő szerint a csoporton belül ===
...
function setupThemeSwitcher() {
    ...
    const setIcon = (theme) => {
        themeSwitcher.innerHTML = theme === 'dark'
            ? '<circle cx="12" cy="12" r="5"></circle><line x1="12" 
y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line>'
...
// Ne fusson hibára, ha nincs meg a konténer
...
// Prepend to be behind other content
        const orbCount = 5;
// Fewer orbs for the header
...
// Ne engedjünk többet kiválasztani
            checkbox.checked = false;
