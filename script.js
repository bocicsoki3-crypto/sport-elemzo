...
sessionStorage.setItem('openingOdds', JSON.stringify(data.odds || {}));
        ...
        addRosterToggleListeners(); // === ÚJ (6 FŐS BIZOTTSÁG) === Hozzáadjuk az új eseményfigyelőt
...
if (chatWrapper) {
        ...
        // (A 'committeeResults' struktúra változni fog, de a logika hasonló marad)
        const { committeeResults, recommendation } = analysisData;
appState.currentAnalysisContext = `Fő elemzés: ${committeeResults.strategic_synthesis || 'N/A'}\n
...
margin-bottom: 1rem; color: var(--primary);">Opcionális P1 xG Felülbírálás (4-Komponensű)</h5>
        <div class="manual-xg-grid" style="margin-top: 0.5rem;">
            ...
            <input type="text" inputmode="decimal" placeholder="V 
xGA" class="xg-input" id="manual-a-xga" title="Vendég Csapat (Away) xGA/90">
        </div>

        <h5 style="margin-top: 1.5rem;
margin-bottom: 1rem; color: var(--primary);">Opcionális P1 Hiányzó Felülbírálás</h5>
        <div class="control-group" style="margin-top: 0.5rem;">
            <label for="manual-abs-home">Hazai kulcshiányzók (vesszővel elválasztva)</label>
            <input id="manual-abs-home" class="xg-input" style="text-align: left;" placeholder="Pl.
Kovács, Nagy"/>
        </div>
        <div class="control-group" style="margin-top: 0.5rem;">
            <label for="manual-abs-away">Vendég kulcshiányzók (vesszővel elválasztva)</label>
            <input id="manual-abs-away" class="xg-input" style="text-align: left;" placeholder="Pl.
Szabó"/>
        </div>
...
'Ismeretlen Liga'}</span>
                                        ...
                                        <input type="text" inputmode="decimal" placeholder="H xG" class="xg-input xg-input-h-xg" title="Hazai Csapat (Home) xG/90">
                                            <input type="text" inputmode="decimal" placeholder="H xGA" class="xg-input 
xg-input-h-xga" title="Hazai Csapat (Home) xGA/90">
        ...
        .forEach(dateKey => {
         html += `<details class="date-section"><summary>${formatDateLabel(dateKey)}</summary>`;
           ...
           <div style="flex-grow:1; cursor: pointer;" onclick="viewHistoryDetail('${safeItemId}')">
                             <div class="list-item-title">${item.home || '?'} – ${item.away ||
'?'}</div>
...
// === ÚJ (6 FŐS BIZOTTSÁG): Eseményfigyelő a P1 Hiányzó legördülőhöz ===
function addRosterToggleListeners() {
    const summaries = document.querySelectorAll('.roster-selector-summary');
    summaries.forEach(summary => {
        // Eltávolítjuk a régi eseményfigyelőt, hogy elkerüljük a duplikálódást
        summary.removeEventListener('click', handleRosterToggle);
        summary.addEventListener('click', handleRosterToggle);
    });
}

// === ÚJ (6 FŐS BIZOTTSÁG): Azonnali keret-lekérő funkció ===
async function handleRosterToggle(event) {
    event.preventDefault(); // Megakadályozza a <details> elem automatikus nyitását/zárását
    
    const summary = event.currentTarget;
    const details = summary.closest('details');
    const matchId = summary.dataset.matchId;
    const container = details.querySelector('.roster-selector-container');
    
    if (!matchId || !container) {
        console.error("Hiba: hiányzó 'data-match-id' vagy '.roster-selector-container' a DOM-ban.");
        return;
    }

    // Csak akkor töltjük be, ha még nincs nyitva
    if (!details.open) {
        // 1. Ellenőrizzük a Cache-t
        const cachedRosters = appState.rosterCache.get(matchId);
        if (cachedRosters) {
            console.log(`P1 Keret: Cache találat (${matchId})`);
            _buildRosterSelectorHtml(cachedRosters, container); // Betöltés a cache-ből
            details.open = true; // Kinyitás
            return;
        }

        // 2. Nincs Cache, API hívás
        const placeholder = container.querySelector('.roster-placeholder');
        if (placeholder) {
            placeholder.textContent = "Keretek lekérése a szerverről...";
        }
        details.open = true; // Kinyitjuk, hogy a user lássa a töltést

        try {
            // Adatok kinyerése a summary gomb data attribútumaiból
            const { home, away, league, kickoff } = summary.dataset;
            ...
            const response = await fetchWithAuth(`${appState.gasUrl}/getRosters`, {
                ...
                body: JSON.stringify({
                    home: unescape(home),
                    away: unescape(away),
                    leagueName: unescape(league),
                    utcKickoff: unescape(kickoff),
                    sport: appState.currentSport
                })
            });
            ...
            // 3. Mentés a Cache-be és Megjelenítés
            appState.rosterCache.set(matchId, rosters);
            _buildRosterSelectorHtml(rosters, container); // HTML felépítése

        } catch (e) {
            console.error(`Hiba a P1 keret lekérésekor (${matchId}):`, e);
            container.innerHTML = `<p class="muted" style="color:var(--danger); font-size: 0.8rem;">A keretek lekérése sikertelen: ${e.message}</p>`;
            details.open = true; // Nyitva tartjuk, hogy a hiba látható legyen
        }
    } else {
        details.open = false; // Ha már nyitva volt, csukja be
    }
}
...
// === MÓDOSÍTÁS (6 FŐS BIZOTTSÁG): Keret-választó UI Építő ===
// Ez a függvény most már a 'containerElement'-be injektálja a HTML-t,
// vagy visszaad egy stringet, ha nincs container megadva.
function _buildRosterSelectorHtml(availableRosters, containerElement = null) {
...
return html;
   ...
   <h5>Vendég Keret (P1 Hiányzók)</h5>
        ...
        <style>
            .roster-selector-grid { display: grid;
grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 0.5rem; }
            .roster-selector-column { max-height: 200px;
overflow-y: auto; background: rgba(0,0,0,0.2); border-radius: 8px; padding: 0.75rem; font-size: 0.85rem;
}
            .roster-position-group { margin-bottom: 0.5rem;
}
            .roster-position-group strong { font-size: 0.75rem; text-transform: uppercase;
color: var(--text-secondary); display: block; margin-bottom: 0.25rem; }
            .roster-checkbox-label { display: block;
margin: 0.25rem 0; cursor: pointer; }
    ...
    if (containerElement) {
        containerElement.innerHTML = finalHtml; // Beinjektáljuk a DOM-ba
    } else {
        return finalHtml; // Visszaadjuk stringként (az elemzési modal számára)
    }
}
...
'N/A'} - ${sim?.topScore?.ga ?? 'N/A'}</strong>`;
    ...
    // === MÓDOSÍTÁS (6 FŐS BIZOTTSÁG) ===
    // Felkészülünk az új bizottsági struktúrára.
    // Ha a régi ('unified') struktúra jön, azt is kezeljük.
    let expertConfHtml, expertConfScore, prophetText, synthesisText, microModelsHtml, quantReportHtml, scoutReportHtml;

    if (fullAnalysisReport && fullAnalysisReport.master_recommendation) {
        // --- A. ESET: Régi (Unified v55.1) Struktúra ---
        expertConfHtml = fullAnalysisReport?.final_confidence_report || "**1.0/10** - Hiba.";
        expertConfScore = 1.0;
        try {
...
if (match && match[1]) { expertConfScore = parseFloat(match[1]); }
        ...
        prophetText = fullAnalysisReport?.prophetic_timeline || "A Próféta nem adott meg jóslatot.";
        ...
        synthesisText = fullAnalysisReport?.strategic_synthesis || "A stratégiai szintézis nem elérhető.";
        microModelsHtml = getMicroAnalysesHtml(fullAnalysisReport?.micromodels, teamNames);
        quantReportHtml = '';
        } else if (fullAnalysisReport && fullAnalysisReport.strategist) {
        ...
        expertConfHtml = strategistReport?.final_confidence_report || "**1.0/10** - Stratéga hiba.";
        expertConfScore = 1.0;
        try {
            const match = expertConfHtml.match(/\*\*(\d+(\.\d+)?)\/10\*\*/);
            if (match && match[1]) { expertConfScore = parseFloat(match[1]); }
        ...
        // A mikromodellek most már a 'strategist' alatt fészkelve érkeznek (ahogy az AnalysisFlow.ts definiálja)
        microModelsHtml = getMicroAnalysesHtml(strategistReport?.micromodels, teamNames);
        
        // A régi Quant/Scout jelentések (ha szükség van rájuk)
        quantReportHtml = (fullAnalysisReport?.quant) ? `
            <div class="committee-card quant">
                ...
                <p><strong>Tiszta xG:</strong> ${fullAnalysisReport.quant.pure_mu_h?.toFixed(2)} - ${fullAnalysisReport.quant.pure_mu_a?.toFixed(2)}</p>
            </div>` : '';
        ...
        scoutReportHtml = (criticReport?.tactical_summary) ? `
            <div class="committee-card scout">
                ...
                <strong>Kulcs Ellentmondások:</strong>
                <ul class="key-insights">
                    ${processAiList(criticReport.key_contradictions, teamNames)}
                </ul>
            </div>` : '';

    } else {
        // --- C. ESET: Hiba ---
        prophetText = "Hiba: Az elemzési jelentés (committeeResults) struktúrája ismeretlen.";
        synthesisText = "Hiba: Az elemzési jelentés (committeeResults) struktúrája ismeretlen.";
        expertConfHtml = "**1.0/10** - Ismeretlen adatszerkezet.";
        expertConfScore = 1.0;
        microModelsHtml = "<p>Hiba</p>";
        quantReportHtml = "<p>Hiba</p>";
        scoutReportHtml = "<p>Hiba</p>";
    }
    ...
    // Ez már _buildRosterSelectorHtml-ként van kezelve, és stringet ad vissza
    const rosterSelectorHtml = _buildRosterSelectorHtml(availableRosters, null); 
