...
async function runAnalysis(home, away, utcKickoff, leagueName, forceNew = false, manualXg = {}) {
    ...
    try {
        ...
        const finalHtml = buildAnalysisHtml_CLIENTSIDE(
            analysisData.committee, // MÓDOSÍTÁS (6 FŐS BIZOTTSÁG): 'committeeResults' -> 'committee'
...
async function viewHistoryDetail(id) {
    ...
    try {
        ...
        if (record.html.startsWith("JSON_API_MODE")) {
            ...
            if (record.html.includes("v63.0 Lánc")) {
                 contentToDisplay = `<p class="muted" style="text-align:center; padding: 2rem;">Ez egy "v63.0 Bizottsági Lánc" elemzés.<br>A mentett JSON adatok visszatöltése és újrarajzolása jelenleg még nincs implementálva.</p>`;
            } else {
                 contentToDisplay = `<p class="muted" style="text-align:center; padding: 2rem;">Ez egy régebbi (v55-v62) JSON API-n keresztül mentett elemzés.<br>A JSON adatok újrafeldolgozása a v63.0 nézethez nem lehetséges.<br><br><i>${escapeHTML(record.html)}</i></p>`;
            }
        } else {
            contentToDisplay = `<div class="analysis-body">${record.html}</div>`; // Régi HTML-alapú mentések
        }
...
function _buildRosterSelectorHtml(availableRosters, containerElement = null) {
    ...
    const finalHtml = `
        ...
        <style>
            ...
            .roster-position-group strong { font-size: 0.75rem; text-transform: uppercase; color: var(--text-secondary); display: block; margin-bottom: 0.25rem; }
...
/**
 * === FŐ KLIENSOLDALI HTML ÉPÍTŐ (ÁTÍRVA v63.0) ===
 */
function buildAnalysisHtml_CLIENTSIDE(
    ...
    if (fullAnalysisReport && fullAnalysisReport.strategist) {
        ...
        expertConfHtml = fullAnalysisReport?.final_confidence_report || "**1.0/10** - Ismeretlen adatszerkezet.";
        expertConfScore = 1.0;
